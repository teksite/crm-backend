@props(['type'=>'submit' , 'color'=>null ,'disabled'=>false])
@php
    $class=match ($color){
...
<button {{$disabled ? 'disabled' : ''}} type="{{$type}}" {{$attributes->merge(['class'=> "rounded text-sm px-3 py-2 shadow font-semibold  select-none $status $class "])}}>
    {!! $slot !!}
</button>
